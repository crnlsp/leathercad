...
| D6 | **"Inward" is decided by winding, not by material.** Right for an outer contour, wrong for a cut-out, whose material is outside the path. Unreachable today, since nothing creates cut-outs. | `applyDerivation` reads the signed area |
| D7 | **Project defaults are ignored.** The panel adds stitch lines at a hard-coded 3.5 mm; `settings.defaultStitchInsetMm` is never read. | `PropertyPanel.tsx`: `DEFAULT_STITCH_INSET_MM` |
